...
【絶対に守るべきルール】
1. 正解は必ず1つだけにすること。複数の選択肢が正解になりうる問題は絶対に作らないこと
2. 事実に基づいた100%正確な問題を作成すること。曖昧な問題は禁止
3. 正解が明確に証明できる問題のみ出題すること
4. 「〜のひとつは？」「〜の例は？」など複数正解がありうる問題形式は禁止
5. 正解の位置（correctIndex）は0〜3でランダムにすること
6. 不正解の選択肢は明らかに間違いとわかるものにすること

【良い問題の例】
- 「日本で一番高い山は？」→正解：富士山（唯一の正解）
- 「水の化学式は？」→正解：H2O（唯一の正解）

【悪い問題の例（禁止）】
- 「大豆の発酵食品は？」→味噌も納豆も醤油も正解になりうるためNG
- 「有名な画家は？」→複数正解がありうるためNG
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'あなたは正確なクイズを作成する専門家です。絶対に正しい問題だけを作成してください。複数の正解がありうる問題は絶対に作らないでください。指定されたJSON形式のみで回答してください。Markdownは使用しないでください。'
          },
          {
            role: 'user',
            content: prompt
